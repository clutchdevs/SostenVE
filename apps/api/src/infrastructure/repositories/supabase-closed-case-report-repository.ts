...
import { reportLabel } from '../../application/reports/labels.js';
...
export class SupabaseClosedCaseReportRepository implements ClosedCaseReportRepository {
  ...
  private toReportRow(row: ClosureReportRow, names: Map<string, string>): ClosedCaseReportRow {
    ...
    return {
      casoId: row.case_id,
      rama: reportLabel.branch(c.branch),
      nivelRiesgo: reportLabel.risk(c.risk_level),
      scoreUrgencia: c.urgency_score,
      tipoSolicitante: reportLabel.requesterType(c.requester_type),
      zona: c.zone,
      modalidad: reportLabel.modality(c.preferred_modality),
      metodoContacto: reportLabel.contactMethod(c.preferred_contact_method),
      edad: c.age,
      sintomasIntake: reportLabel.intakeTags(c.intake_tags ?? []),
      cambioHabitos: reportLabel.habitChanges(c.habit_changes ?? []),
      ...
      motivoNoContacto: reportLabel.noContactReason(row.no_contact_reason),
      sexo: reportLabel.sex(row.sex),
      destinatario: reportLabel.recipient(row.recipient),
      sintomas: reportLabel.symptoms(row.symptoms ?? []),
      otroSintoma: this.safeDecrypt(row.other_symptom_encrypted, 'other_symptom', row.case_id),
      medioContacto: reportLabel.contactMedium(row.contact_medium),
      tecnicas: reportLabel.techniques(row.techniques ?? []),
      motivoCierre: reportLabel.closeReason(row.close_reason),
      derivacionTipo: reportLabel.referralType(row.referral_type),
      derivacionDestinos: reportLabel.referralDestinations(row.referral_destinations ?? []),
      minutos: row.minutes,
